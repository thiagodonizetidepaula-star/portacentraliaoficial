...
// V117 — carrega a correção final do menu categorizado em todas as páginas.
(function loadCategorizedDesktopMenuFix(){
  if(document.querySelector('link[data-menu-v117]')) return;
  const link=document.createElement('link');
  link.rel='stylesheet';
  link.href='assets/css/menu-v117.css?v=117';
  link.dataset.menuV117='true';
  document.head.appendChild(link);
})();

// V119 — Sobre e Contato ficam exclusivamente no rodapé.
(function removeInstitutionalLinksFromHeader(){
  const setup=()=>{
    document.querySelectorAll('.v113-nav a[href="sobre.html"],.v113-nav a[href="contato.html"],.v118-institutional').forEach(el=>el.remove());
  };
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',setup);
  else setup();
})();

// V120 — correção visual exclusiva do enquadramento do menu no computador.
(function loadStableDesktopDropdownFrame(){
  if(document.querySelector('link[data-menu-v120]')) return;
  const link=document.createElement('link');
  link.rel='stylesheet';
  link.href='assets/css/menu-v120.css?v=120';
  link.dataset.menuV120='true';
  document.head.appendChild(link);
})();
